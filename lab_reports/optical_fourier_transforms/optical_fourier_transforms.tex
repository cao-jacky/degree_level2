...
\section{Method} 
\vspace{-2ex}
To create and observe diffraction patterns we needed to set-up the $4f$ system. Figure \ref{m-fig1} shows the experimental setup that was used. It is worth noting that each component was attached to an optical breadboard.
\begin{figure}[!h]
\begin{center}
\includegraphics[width=6.5cm]{method/fig1-1}
\caption[]{A plan view schematic of the experimental set-up used to collect data. $f$ is the focal length distances of the lenses. (a) Represents the main $2f$ optical system used where the majority of data was collected. (b) Represents the 'standard' $4f$ optical system.}
\label{m-fig1}
\end{center}
\end{figure}

A coherent laser source of wavelength $635\pm?$ nm was used as our source of radiation, a $\times10$ beam expander was placed directly after this to allow the beam width to be adjusted during experimentation. 

To place the lenses at the correct distances their focal lengths had to be measured. This was done by placing each lens at the beam expander, then moving a screen towards and away from the source until the brightest and sharpest spot could be observed. At that distance was thus the focal length, measured with a metre ruler. We found that for both lens 1 and 2, their focal lengths were $48\pm1$ cm.

Due to the constraint with the size of the breadboard, mirrors were used to reflect the light so that the set-up would take up less space. Two were positioned so that light could be reflected into a CCD camera. 

This camera was moved at various stages of experimentation, however what concerned us the most was the camera being at the Fourier plane, a distance of $2f$. It was later moved to $4f$ when we wanted observations  at that distance.

Our CCD was controlled using software, images were saved and then later analysed using custom built modules compiled in Python and in MATLAB. The output was an intensity map of the image, and graphs detailing the intensity profiles in the $x$ and $y$ directions.  

Diffraction gratings were placed within a holder, and then the pattern would be observed at the different distances. These gratings included single slits, coarse gratings, and custom shapes such as double circular apertures. 

Before images were saved and processed, adjustments had to be made so that the pattern observed was as clear as possible. One recurring problem was the over saturation of the image, this was accounted for by using a neutral density filter. This removed some of the light, so producing a cleaner image. Other actions were also taken to reduce saturation, for example adjusting the exposure time and gain within the software.
