...
Discovered by Poiseulle in whenver, and Hagen in whenver, for a liquid (fluid?) flowing out of a tube of density, at height, and through a tube of radius and length, the flow rate can be expressed functionally.  

When considering fluids, amongst other properties one which we can study is the viscosity of the fluid. The viscosity of a fluid can be defined as the internal friction within it. This results in shear stress when two adjacent layers of fluid move relative to each other. 

If we consider a fluid to be moving in a single direction, as shown in figure (whatever), then we can say that as the top layer (lamina) moves across, it slides over another lamina - this causes the streamlines for the lamina flow to be smooth. 

In situations where the a fluid is considered be incompressible, if we allow the flthrough various measurements of the properties attributed with the liquid, we can calculate a value for the viscosity of the fluid. 

As disocvered by Poiseeule in whenever, and Hagen in whenenver, the following equation relates the flow rate ($\frac{dV}{dt}$) to vaksjndas
